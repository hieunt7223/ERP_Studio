    ...
    public class ExcelExport
    {
        ...
        public static void CreateExcelComponent(string temFilePath, string newFilePath)
        {
            try
            {
                ...
                _temPackage = new ExcelPackage(new FileInfo(newFilePath));
        ...
        #region CustomCodeColumnDinamic
        public static void AddColumnDynamic(string temFilePath, string newFilePath, DataSet ds)
        {
            CreateExcelComponent(temFilePath, newFilePath);
            if (_workSheet != null && _tempSheet != null)
            {
                if (ds != null && ds.Tables.Count > 0)
                {
                    foreach (DataTable dtColumns in ds.Tables)
                    {
                        ...
                        foreach (DataRow hRow in dtColumns.Rows)
                        {
                            int rowIndex = Convert.ToInt32(hRow["HeadedRowIndex"]);
                            int temCol = Convert.ToInt32(hRow["ColumnIndex"]);
                            if (newCol == 0)
                            {
                                newCol = temCol;
                            }
                            ...
                            {
                                _workSheet.InsertColumn(newCol, 1, temCol);
                                _workSheet.Column(newCol).Width = _workSheet.Column(temCol).Width;
                            }
                            ExcelRange temCell = _workSheet.Cells[rowIndex, temCol];
                            ExcelRange newCell = _workSheet.Cells[rowIndex, newCol];
                            newCell.Value = hRow["Description"];
                            newCell.StyleID = temCell.StyleID;

                            //Total
                            int TotalRowIndex = Convert.ToInt32(hRow["TotalRowIndex"]);
                            if (TotalRowIndex > 0)
                            {
                                ExcelRange TotaltemCell = _workSheet.Cells[TotalRowIndex, temCol];
                                ExcelRange TotalnewCell = _workSheet.Cells[TotalRowIndex, newCol];
                                TotalnewCell.Value = "{" + hRow["Key"] + "}";
                                TotalnewCell.StyleID = TotaltemCell.StyleID;
                            }
                            //Detail
                            int detailRowIndex = Convert.ToInt32(hRow["DetailRowIndex"]);
                            if (detailRowIndex > 0)
                            {
                                ExcelRange detailtemCell = _workSheet.Cells[detailRowIndex, temCol];
                                ExcelRange detailnewCell = _workSheet.Cells[detailRowIndex, newCol];
                                detailnewCell.Value = "{" + hRow["Key"] + "}";
                                detailnewCell.StyleID = detailtemCell.StyleID;
                            }
                            newCol++;
                        }
                        //Merge của các cột vừa group
                        int firstRow = Convert.ToInt32(dtColumns.Rows[0]["GroupRowIndex"]);
                        int firstCol = Convert.ToInt32(dtColumns.Rows[0]["ColumnIndex"]);
                        ExcelRange newColRange = _workSheet.Cells[firstRow, firstCol,
                            firstRow, firstCol + dtColumns.Rows.Count - 1];
                        //newColRange.Value = null;
                        newColRange.Merge = true;
                        //Trả lại giá trị ban đầu của cell của template
                        //newColRange.Value = _workSheet.Cells[firstRow, firstCol].Value;
                    }
                }
            }
        }

        public static void ExportWithTemplateCustom(string temFilePath, string newFilePath, DataTable dtHeader, DataTable dtData, DataTable dtSumData, int sheetIndex, int hiddenColumns)
        {
            _sheetIndex = sheetIndex;
            var dSet = new DataSet();
            if (dtHeader != null && dtHeader.Rows.Count > 0)
            ...
            ExportWithTemplateCustom(temFilePath, newFilePath, dSet, hiddenColumns);
            if (dSet.Tables != null && dSet.Tables.Count > 0)
                dSet.Tables.Clear();
        }

        public static void ExportWithTemplateCustom(string temFilePath, string newFilePath, DataSet dSet, int hiddenColumns)
        {
            if (_workSheet != null && _tempSheet != null)
            {
                if (dSet != null && dSet.Tables.Count > 0)
