...
namespace xdcb.Common.DanhMuc.LoaiHoSos
{
    public class EditModalModel : Web.Pages.CommonBaseAbpPageModel
    {
        ...
        {
            if (!await _authorization.IsGrantedAsync(xdcbCommonPermissions.LoaiHoSoPermission.Update))
            {
                throw new AbpAuthorizationException(Messages.MSG_NotPermission);
            }
            Id = loaiHoSoId;
            var loaiHoSoDto = await _loaiHoSoAppService.GetLoaiHoSoByIdAsync(Id);
