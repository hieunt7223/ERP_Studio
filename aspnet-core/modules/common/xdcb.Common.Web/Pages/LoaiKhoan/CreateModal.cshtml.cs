...
namespace xdcb.Common.DanhMuc.LoaiKhoans
{
    public class CreateModalModel : AbpPageModel
    {
        ...
        public CreateModalModel(ILoaiKhoanAppService loaiKhoanAppService, IAuthorizationService authorization)
        {
        ...
        {
            if (await _authorization.IsGrantedAsync(xdcbCommonPermissions.LoaiKhoanPermission.Create))
            {
                var loaiKhoanDto = ObjectMapper.Map<LoaiKhoanViewModel, CreateUpdateLoaiKhoanDto>(LoaiKhoan);
                await _loaiKhoanAppService.CreateAsync(loaiKhoanDto);
                return NoContent();
            }
