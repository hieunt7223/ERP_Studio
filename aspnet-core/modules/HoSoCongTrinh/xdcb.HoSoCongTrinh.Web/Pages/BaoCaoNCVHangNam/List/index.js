$(function () {
    ...
    var dataTable = $('#BaoCaoTable').DataTable(abp.libs.datatables.normalizeConfiguration({
        ...
        ajax:
            abp.libs.datatables.createAjax(function () {
                return abp.ajax($.extend(true, {
                    url: "/api/app/nhuCauKeHoachVon/search?Tenkehoach=HANG_NAM&ChuDauTuId=" + chuDauTuId + "&SkipCount=" + $('#BaoCaoTable').DataTable().page.info().page * $('#BaoCaoTable').DataTable().page.len() + "&MaxResultCount=" + $('#BaoCaoTable').DataTable().page.len(),
                    type: "get"
                }));
        ...
        columnDefs: [
            ...
            { className: "text_center", data: "tuNam" },
        {
            className: "text_center",
                data: { id: "id", loaiKeHoach: "loaiKeHoach",tuNam:"tuNam"}, "render": function (data) {
                    if (data.loaiKeHoach === 'DAU_NAM') {
                        let str = '<a href="/BaoCaoNCVHangNam/Detail?keHoachNhuCauVonId=' + data.id + '&&loaiKeHoach=' + data.loaiKeHoach + '&&year=' + data.tuNam + '">' + "Đầu Năm" + '</a>'
                        return str;
                    } else {
                        str = '<a href="/BaoCaoNCVHangNam/Detail?keHoachNhuCauVonId=' + data.id + '&&loaiKeHoach=' + data.loaiKeHoach + '&&year=' + data.tuNam + '">' + "Điều chỉnh" + '</a>'
                        return str;
                    }
                }
            },
            ...
            },
            {
                ...
                data: { id: "id", trangThaiDauNam: "trangThaiDauNam", loaiKeHoach: "loaiKeHoach", trangThaiDieuChinh: "trangThaiDieuChinh", tuNam: "tuNam" },
                "render": function (data) {
                    var str = '';
                    str = '<a href="/BaoCaoNCVHangNam/Edit?keHoachNhuCauVonId=' + data.id + '&&loaiKeHoach=' + data.loaiKeHoach + '&&year=' + data.tuNam + '"><button class="btn-action btn-edit" type="button"><i class="fa fa-pencil-square-o"></i></button></a>';
    ...
    function Search() {
        ...
        dataTable = $('#BaoCaoTable').DataTable(abp.libs.datatables.normalizeConfiguration({
            ...
            ajax: abp.libs.datatables.createAjax(function () {
                return abp.ajax($.extend(true, {
                    url: "/api/app/nhuCauKeHoachVon/search?Nam=" + $('#Year').val() + "&&Loaikehoach=" + $('#LoaiKeHoach').val() + "&&Tenkehoach=HANG_NAM&&ChuDauTuId=" + chuDauTuId + "&&SkipCount=" + $('#BaoCaoTable').DataTable().page.info().page * $('#BaoCaoTable').DataTable().page.len() + "&&MaxResultCount=" + $('#BaoCaoTable').DataTable().page.len(),
                    type: "get"
                }));
